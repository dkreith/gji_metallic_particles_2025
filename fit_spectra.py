...
c3_m4 = fit_c3(c0*np.sqrt(poro/form), T, epsr, mu/np.sqrt(form*poro), a, alpha,
               beta, nu, w_exp, sig_exp*1e-3)
sig_m4 = wong(w_mod, c0*np.sqrt(poro/form), c3_m4, T, epsr,
              mu/np.sqrt(form*poro), a, alpha, beta, nu, mod="sig")
...
ax[1,1].semilogx(w_mod/2/np.pi, 1e3*sig_m3.imag, "--", linewidth=2.5,
                 color="tab:orange")
ax[0,1].semilogx(w_mod/2/np.pi, 1e3*sig_m4.real, ":", linewidth=2.5,
                 color="tab:green")
ax[1,1].semilogx(w_mod/2/np.pi, 1e3*sig_m4.imag, ":", linewidth=2.5,
                 color="tab:green")
...
ax[0,1].legend(["Measured data",
                ...
                "Response with turtuosity $T_1$",
                "Response with turtuosity $T_2$",], loc=9)
...
fig.tight_layout()
fig.savefig("figure_10.pdf", dpi=300)